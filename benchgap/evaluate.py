...
This is the canonical source. It was extracted unchanged from three
independent studies that had each arrived at the same code, and those
studies now import it instead of carrying copies.

Why that mattered: the multi-domain claim those studies make -- that the
kind of distribution shift predicts how far a benchmark score falls in
deployment -- is a comparison between numbers. It is only a comparison if
every number is the same quantity computed the same way. Three copies kept
in sync by a hash check would drift eventually, and when they did the
comparison would quietly stop being one, with nothing to warn you.
"""
